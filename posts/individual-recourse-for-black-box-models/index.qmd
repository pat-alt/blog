...
dt_toy[,emoji:=sprintf("posts/individual-recourse-for-black-box-models/www/%s.png",tolower(type))]
...
anim_save("posts/individual-recourse-for-black-box-models/www/toy.gif")
```
...
p <- ggplot(data=dt_plot, aes(x=value)) +
  ...
  geom_image(data=kitty_dt, aes(x=value, y=c(0.05,0.20)), image="posts/individual-recourse-for-black-box-models/www/cat.png", size=0.1) +
  labs(
    x=NULL,
...
```{r fit}
source("posts/individual-recourse-for-black-box-models/R/linear_classifier.R")
n_iter <- 1000 
...
dt_plot[,alpha:=1]
dt_plot[,emoji:=sprintf("posts/individual-recourse-for-black-box-models/www/%s.png",tolower(y_hat))]
p <- ggplot() +
...
```{r}
set.seed(seed)
source("posts/individual-recourse-for-black-box-models/R/revise.R")
lambda <- c(
...
```{r, eval=FALSE}
steps[,emoji:=sprintf("posts/individual-recourse-for-black-box-models/www/%s.png",tolower(y_hat))]
p <- p +
...
anim_save("posts/individual-recourse-for-black-box-models/www/revise.gif")
```
...
knitr::include_graphics("posts/individual-recourse-for-black-box-models/www/revise.gif")
```
